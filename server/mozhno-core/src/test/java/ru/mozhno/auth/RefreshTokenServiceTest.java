...
class RefreshTokenServiceTest {
    ...
    void issueTokens_rememberMe_shouldUseLongerTtl() {
        User user = createTestUser();
        when(jwtProperties.getRefreshTokenTtlDays()).thenReturn(30L);
        when(jwtService.generateAccessToken(user)).thenReturn("access.token.2");
        when(refreshTokenRepository.save(any(RefreshToken.class))).thenAnswer(inv -> {
            RefreshToken t = inv.getArgument(0);
            t.setId(2);
            return t;
    ...
    void issueTokens_shouldStoreSha256Hash() {
        User user = createTestUser();
        when(jwtProperties.getRefreshTokenTtlDays()).thenReturn(30L);
        when(jwtService.generateAccessToken(user)).thenReturn("access.token.3");
        when(refreshTokenRepository.save(any(RefreshToken.class))).thenAnswer(inv -> {
            RefreshToken t = inv.getArgument(0);
            t.setId(3);
            return t;
        ...
        assertEquals(RefreshTokenService.sha256(pair.getRefreshToken()), saved.getTokenHash());
    ...
    void refresh_shouldIssueNewTokenPair() {
        ...
        when(refreshTokenRepository.findByHash(oldHash)).thenReturn(existingToken);
    ...
    void refresh_shouldMarkOldTokenAsUsed() {
        ...
        when(refreshTokenRepository.findByHash(oldHash)).thenReturn(existingToken);
        when(userRepository.findById(user.getId())).thenReturn(user);
        when(jwtService.generateAccessToken(user)).thenReturn("acc.tok");
        when(refreshTokenRepository.save(any(RefreshToken.class))).thenAnswer(inv -> {
            RefreshToken t = inv.getArgument(0);
    ...
    void refresh_shouldThrowOnUnknownToken() {
        ...
        String unknownHash = RefreshTokenService.sha256(unknownRaw);

        when(refreshTokenRepository.findByHash(unknownHash)).thenReturn(null);
    ...
    void refresh_shouldThrowOnRevokedToken() {
        ...
        when(refreshTokenRepository.findByHash(hash)).thenReturn(revokedToken);
    ...
    void refresh_shouldThrowOnExpiredToken() {
        ...
        when(refreshTokenRepository.findByHash(hash)).thenReturn(expiredToken);
    ...
    void refresh_shouldThrowAndRevokeFamilyOnReuse() {
        ...
        when(refreshTokenRepository.findByHash(hash)).thenReturn(usedToken);
    ...
    void sha256_shouldProduceDeterministicHash() {
        ...
        String hash1 = RefreshTokenService.sha256(input);
        String hash2 = RefreshTokenService.sha256(input);
    ...
    void sha256_shouldProduceDifferentHashForDifferentInputs() {
        String hash1 = RefreshTokenService.sha256("input-one");
        String hash2 = RefreshTokenService.sha256("input-two");
    ...
    void issueTokens_shouldGenerateUniqueRefreshes() {
        User user = createTestUser();
        when(jwtProperties.getRefreshTokenTtlDays()).thenReturn(30L);
        when(jwtService.generateAccessToken(user)).thenReturn("access.1", "access.2");
        when(refreshTokenRepository.save(any(RefreshToken.class))).thenAnswer(inv -> {
            RefreshToken t = inv.getArgument(0);
            t.setId(100);
            return t;
