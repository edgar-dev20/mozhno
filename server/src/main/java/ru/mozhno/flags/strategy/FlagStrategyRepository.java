...
public class FlagStrategyRepository {
    ...
    public List<FlagStrategy> findByFlagId(Integer flagId) {
        return jdbc.query("SELECT id, flag_id, environment_id, strategy_type, enabled, percentage, rollout_percentage, context_definition_id, context_values_json, segment_id, created_at FROM flag_strategies WHERE flag_id = ?", ROW_MAPPER, flagId);
    }

    public FlagStrategy findById(Integer id) {
        try {
            return jdbc.queryForObject("SELECT id, flag_id, environment_id, strategy_type, enabled, percentage, rollout_percentage, context_definition_id, context_values_json, segment_id, created_at FROM flag_strategies WHERE id = ?", ROW_MAPPER, id);
    ...
    public FlagStrategy findByFlagIdAndEnvironmentId(Integer flagId, Integer environmentId) {
        try {
            return jdbc.queryForObject("SELECT id, flag_id, environment_id, strategy_type, enabled, percentage, rollout_percentage, context_definition_id, context_values_json, segment_id, created_at FROM flag_strategies WHERE flag_id = ? AND environment_id = ?", ROW_MAPPER, flagId, environmentId);
        } catch (org.springframework.dao.EmptyResultDataAccessException e) {
            return null;
        }
    }
    ...
    public FlagStrategy save(FlagStrategy fs) {
        if (fs.getId() == null) {
            ...
            jdbc.update("INSERT INTO flag_strategies (flag_id, environment_id, strategy_type, enabled, percentage, rollout_percentage, context_definition_id, context_values_json, segment_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ...
            jdbc.update("UPDATE flag_strategies SET environment_id = ?, strategy_type = ?, enabled = ?, percentage = ?, rollout_percentage = ?, context_definition_id = ?, context_values_json = ?, segment_id = ? WHERE id = ?",
