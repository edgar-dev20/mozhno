...
<div class="nav-section">
    ...
    document.querySelectorAll(".btn-delete-tag").forEach((btn) => {
      btn.onclick = (e) => {
        e.stopPropagation();
        deleteTag(parseInt(btn.dataset.id));
      };
    });
  ...
  async function renderFlag(container) {
                ...
                <main class="flag-main">
                    ${isEditing ? `
                        ...
                        <div class="form-group">
                            ...
                            <div class="tag-values-list">${tags.length === 0 ? '<span class="empty-hint">No tags</span>' : tags.slice(0, 10).map((t) => {
    ...
    if (document.getElementById("btnEditFlag")) {
      document.getElementById("btnEditFlag").onclick = () => {
        window.editingFlagId = state.currentFlagId;
        render();
      };
    }
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <div class="form-group">
                ...
                <div class="tag-values-list">${tags.length === 0 ? '<span class="empty-hint">No tags yet</span>' : tags.slice(0, 10).map((t) => `
                        <div class="tag-value-row" data-tag-id="${t.id}">
                            <span class="tag-color-dot" style="background:${t.color}"></span>
                            <span class="tag-name">${escapeHtml(t.name)}</span>
  ...
  window.createFlag = async function() {
    ...
    if (!name || !key) return alert("Name and key required");
    try {
      await api(`/projects/${state.currentProjectId}/flags`, "POST", { name, key, description, flagType, tags: tagValues });
      window.closeModal();
      render();
    } catch (e) {
      alert(e.message);
  ...
  async function deleteTag(id) {
    ...
    try {
      await api(`/projects/${state.currentProjectId}/tags/${id}`, "DELETE");
      render();
    } catch (e) {
      alert(e.message);
    }
