...
export function useFlagFilters(flags: FlagView[], totalItems?: number) {
  ...
  useEffect(() => { setDisplayLimit(10); }, [searchQuery, selectedTagTypeFilter, selectedTagValueFilter, dateFrom, dateTo, sortBy, flagTypeFilter]);
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedTagTypeFilter) {
      result = result.filter(f =>
        f.tags.some(tg => tg.tagId === selectedTagTypeFilter && (!selectedTagValueFilter || tg.value === selectedTagValueFilter))
      );
    }
    if (flagTypeFilter) {
    ...
    return result;
  }, [flags, selectedTagTypeFilter, selectedTagValueFilter, flagTypeFilter, searchQuery, sortBy, dateFrom, dateTo]);
  ...
  const uniqueTagValues = (typeId: number) =>
    [...new Set(flags.flatMap(f => f.tags.filter(t => t.tagId === typeId).map(t => t.value)))].sort();

  return {
    filtered, archivedFlags, visibleFlags, hasMoreFlags,
    showMoreFlags, showAllFlags, displayLimit,
