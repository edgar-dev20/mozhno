    ...
    <div className="border border-border rounded-xl overflow-hidden">
      <div className="flex items-center justify-between gap-1.5 px-3 py-2 bg-secondary border-b border-border">
        ...
        <button
          type="button"
          onClick={copyCurl}
          className="flex items-center gap-1 px-2.5 py-1 text-caption font-medium text-muted-foreground hover:text-brand hover:bg-brand/10 rounded-lg transition-colors"
        >
          {copied ? <Check size={11} /> : <Clipboard size={11} />}
          {copied ? t('integrations.copied') : t('integrations.copy')}
        </button>
      </div>
      <pre className="p-3 bg-input-background text-caption text-foreground/80 font-mono whitespace-pre-wrap break-all m-0 overflow-x-auto">
        {curlCommand}
      </pre>
    </div>
  );
