...
import { formatDate, getFlagTypeColor, getFlagTypeLabel, adjustColor } from "@/shared";
...
const getTypeIcon = (t: string): React.ReactNode => {
  switch (t) {
    ...
    case 'KILLSWITCH':
      return <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>;
    default:
      return null;
  }
};
                ...
                {flag.tags.length > 0 && (
                  <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                    {flag.tags.map((tv, i) => {
                      ...
                      return tg
                        ? <span key={i} className="inline-flex items-center px-2 py-1 rounded text-xs font-medium text-white shadow-sm leading-none" style={{ backgroundImage: `linear-gradient(to right, ${tg.color}, ${adjustColor(tg.color, 20)})` }}>{tv.value}</span>
                        : null;
                    })}
                  </div>
                )}
