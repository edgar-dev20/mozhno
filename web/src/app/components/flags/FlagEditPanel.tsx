  ...
  return (
    <form id="flag-edit-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <FormField label={t('common.key')} hint={t('flags.keyHint')}>
        <input type="text" value={flag.key} disabled className="w-full bg-input-background border border-border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring/50 focus:border-ring transition-all placeholder:text-muted-foreground disabled:opacity-50 font-mono" />
      </FormField>
      ...
      <div className="pt-6 border-t border-border space-y-3">
        {flag.archived
          ? (
            <button type="button" onClick={onUnarchive} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-brand hover:bg-brand/10 rounded-lg border border-brand/20">
              <ArchiveRestore size={16} />{t('flags.unarchiveFlag')}
            </button>
          )
          : (
            <button type="button" onClick={onArchive} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-brand hover:bg-brand/10 rounded-lg border border-brand/20">
              <Archive size={16} />{t('flags.archiveFlag')}
            </button>
          )}
        <button type="button" onClick={onDelete} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-destructive hover:bg-destructive/10 rounded-lg border border-destructive/20">
