  ...
  return (
    <form id="flag-edit-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <FormField label={t('common.key')} hint={t('flags.keyHint')}>
        <input type="text" value={flag.key} disabled className="w-full bg-white dark:bg-neutral-950 border border-border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring/50 focus:border-ring transition-all placeholder:text-muted-foreground disabled:opacity-50 font-mono" />
      </FormField>
      ...
      <div className="pt-6 border-t border-border space-y-3">
        {flag.archived
          ? (
            <button type="button" onClick={onUnarchive} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-500/10 rounded-lg border border-violet-200 dark:border-violet-500/20">
              <ArchiveRestore size={16} />{t('flags.unarchiveFlag')}
            </button>
          )
          : (
            <button type="button" onClick={onArchive} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-500/10 rounded-lg border border-violet-200 dark:border-violet-500/20">
              <Archive size={16} />{t('flags.archiveFlag')}
            </button>
          )}
        <button type="button" onClick={onDelete} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg border border-red-200 dark:border-red-500/20">
