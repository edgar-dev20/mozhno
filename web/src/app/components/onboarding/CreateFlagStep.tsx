...
export function CreateFlagStep({
  ...
  return (
    <div className="space-y-3 flex-1">
      ...
      <div>
        <label
          htmlFor="onboarding-flag-key"
          className="block text-body-sm font-medium text-foreground/80 mb-1.5"
        >
          {t('onboarding.flagKeyLabel')}
        </label>
        <input
          id="onboarding-flag-key"
          type="text"
          value={flagKey}
          onChange={(e) => setFlagKey(e.target.value)}
          maxLength={100}
          placeholder={t('onboarding.flagKeyPlaceholder')}
          className="w-full bg-input-background border border-border text-foreground rounded-lg px-4 py-2.5 text-body-sm focus:outline-none focus:ring-2 focus:ring-ring/50 focus:border-ring transition-all placeholder:text-muted-foreground font-mono"
        />
        <div className="text-caption font-normal text-muted-foreground tabular-nums text-right mt-0.5">
          {flagKey.length}/100
        </div>
