...
describe('useFlagPanels', () => {
  ...
  it('opens create panel', () => {
    ...
    const { result } = renderHook(() =>
      useFlagPanels(1, [], mocks.deleteFlag, mocks.archiveFlag, mocks.unarchiveFlag, mocks.toggleFlagMutation),
    );
  ...
  it('opens general panel', () => {
    ...
    act(() => { result.current.openGeneral(makeFlag()); });
    expect(result.current.panelOpen).toBe(true);
  ...
  it('opens environment panel', () => {
    const mocks = makeMocks();
    const flag = makeFlag({ environments: { 1: { enabled: true, percentage: 50, segmentIds: [1], strategyId: null, contextDefinitionId: null, contextValuesJson: null } } });
    const { result } = renderHook(() =>
      useFlagPanels(1, [], mocks.deleteFlag, mocks.archiveFlag, mocks.unarchiveFlag, mocks.toggleFlagMutation),
    );
    act(() => { result.current.openEnvironment(flag, 1); });
  ...
  it('detects env dirty state', () => {
    const mocks = makeMocks();
    const flag = makeFlag({ environments: { 1: { enabled: false, percentage: 100, segmentIds: [], strategyId: null, contextDefinitionId: null, contextValuesJson: null } } });
    const { result } = renderHook(() =>
      useFlagPanels(1, [], mocks.deleteFlag, mocks.archiveFlag, mocks.unarchiveFlag, mocks.toggleFlagMutation),
    );
    act(() => { result.current.openEnvironment(flag, 1); });
    expect(result.current.isEnvDirty).toBe(false);
    act(() => { result.current.setEnvRulePercent(50); });
  ...
  it('doDelete calls mutateAsync', async () => {
    ...
    act(() => { result.current.setDeleteTarget(makeFlag()); });
  ...
  it('doArchive calls mutateAsync', async () => {
    ...
    act(() => { result.current.setArchiveTarget(makeFlag()); });
  ...
  it('doToggleFlag toggles enabled state', async () => {
    ...
    expect(mocks.toggleFlagMutation.mutateAsync).toHaveBeenCalledWith(expect.objectContaining({ enabled: true }));
  });
