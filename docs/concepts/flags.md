...
Детерминированное распределение на основе MurmurHash32 от `flagKey + userId` (или `sessionId`, либо автоматически сгенерированного `anonymousId`). Один и тот же пользователь всегда попадает в одну группу. Подробнее — [Роллаут](/guide/rollout).
