...
Один и тот же идентификатор всегда получает одинаковый результат для одного флага. В качестве идентификатора используется `userId`, затем `sessionId`.

> **Важно:** Если не передан ни `userId`, ни `sessionId`, хеш считается только по ключу флага — все анонимные пользователи попадают в одну группу (либо все получают фичу, либо никто). SDK выводит предупреждение в лог.
