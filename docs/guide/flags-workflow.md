...
> **Совет:** Ключ флага — это его идентификатор в коде (`isEnabled("new-checkout", ctx)`). Делайте ключи осмысленными. Избегайте `flag-42`. Рекомендации по именованию — [Лучшие практики](/guide/best-practices#именование-флагов).

## Изменение флага

Все поля кроме **ключа** можно редактировать. Каждое изменение записывается в [аудит](/guide/audit).
...
Каждый этап — это изменение правил активации на конкретном окружении. Канареечный запуск и постепенная раскатка в production — с мониторингом метрик на каждом шаге.

Пример одного из возможных сценариев — адаптируйте под свою команду:
...
Теги — это метаданные флага в формате `ключ:значение`. Они позволяют группировать и фильтровать флаги независимо от имени. Один флаг может иметь несколько тегов.

### Зачем нужны теги

У вас 50 флагов. Ключи осмысленные, но найти все флаги команды оформления заказа — невозможно без тегов. Теги решают эту проблему:

| Задача | Без тегов | С тегами |
|--------|-----------|----------|
| Найти все флаги checkout-команды | Пролистать все 50 флагов глазами | Фильтр `team:checkout` — 3 флага |
| Показать все аварийные выключатели | Искать по префиксу `kill-` в ключах | Фильтр `type:killswitch` надёжнее |
| Найти флаги конкретного сервиса | Гадать по названиям | Фильтр `service:api` |

### Как создавать теги

Теги создаются в разделе **Теги** в боковом меню панели. Каждый тег — это пара:
...
После создания тег можно присвоить любому флагу при создании или редактировании.

### Рекомендуемые категории

| Категория | Формат | Примеры |
|-----------|--------|---------|
| Команда | `team:название` | `team:checkout`, `team:platform` |
| Тип флага | `type:назначение` | `type:killswitch`, `type:experiment` |
| Сервис | `service:имя` | `service:api`, `service:payments` |
| Статус | `status:состояние` | `status:deprecated`, `status:permanent` |

На одном флаге может быть несколько тегов: `team:checkout`, `type:killswitch`, `service:api` — флаг одновременно принадлежит команде, классифицирован по типу и привязан к сервису.
