...
Аудит-лог фиксирует **каждое изменение** в системе: кто, что, когда и какие именно поля изменил. **можно**<span class=brand-dot>.</span> хранит полную историю изменений флагов, сегментов, API-ключей и настроек.
...
Для каждого изменения **можно**<span class=brand-dot>.</span> показывает **diff** — что именно поменялось.
...
> **Enterprise-функция:** Экспорт аудит-лога в CSV/JSON/PDF доступен в Enterprise-версии **можно**<span class=brand-dot>.</span> через `FeatureGateSpi.audit.export`. Подробнее — [Open Core](/advanced/open-core).
...
**можно**<span class=brand-dot>.</span> отправляет уведомления о важных изменениях:
