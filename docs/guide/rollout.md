...
В качестве идентификатора используется `userId`, затем `sessionId`. Если не передан ни один — все анонимные пользователи попадают в одну группу. SDK выводит предупреждение в лог.
