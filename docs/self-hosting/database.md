...
Настройка PostgreSQL для **можно**<span class=brand-dot>.</span>: миграции Flyway, пул соединений HikariCP, стратегии резервного копирования, откат миграций, версионирование схемы и рекомендации по индексам.

## Архитектура доступа к данным

**можно**<span class=brand-dot>.</span> использует прямой доступ к базе данных через Spring `JdbcTemplate` — без JPA, Hibernate или другого ORM. Все SQL-запросы пишутся явно, RowMapper'ы преобразуют строки в Java-объекты. Это даёт:
...
Для **можно**<span class=brand-dot>.</span> с 8 подами и лимитом 2 ядра на под:
...
Каждый релиз **можно**<span class=brand-dot>.</span> включает новую версию схемы базы данных. Миграции накапливаются последовательно и никогда не изменяются задним числом — это гарантирует воспроизводимость развёртывания на любой версии.
