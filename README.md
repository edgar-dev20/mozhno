...
**Модель доступа:** активный проект хранится в JWT (claim `project_id`).
Все ресурсы (флаги, сегменты, контексты, API-ключи, аудит и т.д.) ограничены этим
проектом; права на изменение зависят от роли (ADMIN/DEVELOPER/VIEWER).
Проект создаётся один раз при bootstrap и не может быть удалён — используйте сброс.
